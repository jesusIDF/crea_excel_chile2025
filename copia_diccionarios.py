...
########### DEFINICION DE HOJAS BASE ##################
# copias fieles
hoja_make_base : Worksheet = wb_base["Make"]
hoja_body_base : Worksheet = wb_base["Body"]
hoja_drive_base : Worksheet = wb_base["Drive"]
hoja_submodel_base : Worksheet = wb_base["Submodel"]
hoja_fuel_base : Worksheet = wb_base["Fuel"]
hoja_brakesystem_base : Worksheet = wb_base["Brake System"]
hoja_braketype_base : Worksheet = wb_base["Brake Type"]
hoja_brakeabs_base : Worksheet = wb_base["Brake ABS"]
hoja_springtype_base : Worksheet = wb_base["Spring Type"]
hoja_steeringsystem_base : Worksheet = wb_base["Steering System"]
# filtrados
...
hoja_vtoengine_base : Worksheet = wb_base["V To Engine"]
hoja_vtotransmission_base : Worksheet = wb_base["V To Transmission"]
...
# Crear o reemplazar hoja destino Drive
if "Submodel" in wb_destino.sheetnames:
    del wb_destino["Submodel"]
hoja_submodel_destino = wb_destino.create_sheet("Submodel")

# Copiar contenido celda por celda (incluyendo estilo)
for fila in hoja_submodel_base.iter_rows():
    for celda in fila:
        nueva_celda = hoja_submodel_destino.cell(row=celda.row, column=celda.column, value=celda.value)
        if celda.has_style:
            nueva_celda.font = copy(celda.font)
            nueva_celda.border = copy(celda.border)
            nueva_celda.fill = copy(celda.fill)
            ...
            nueva_celda.protection = copy(celda.protection)
            nueva_celda.alignment = copy(celda.alignment)
print("✓ Hoja Submodel creada")

# Crear o reemplazar hoja destino Fuel
if "Fuel" in wb_destino.sheetnames:
    del wb_destino["Fuel"]
hoja_fuel_destino = wb_destino.create_sheet("Fuel")

# Copiar contenido celda por celda (incluyendo estilo)
for fila in hoja_fuel_base.iter_rows():
    for celda in fila:
        nueva_celda = hoja_fuel_destino.cell(row=celda.row, column=celda.column, value=celda.value)
        if celda.has_style:
            nueva_celda.font = copy(celda.font)
            nueva_celda.border = copy(celda.border)
            nueva_celda.fill = copy(celda.fill)
            ...
            nueva_celda.protection = copy(celda.protection)
            nueva_celda.alignment = copy(celda.alignment)
print("✓ Hoja Fuel creada")

# Crear o reemplazar hoja destino Brake System
if "Brake System" in wb_destino.sheetnames:
    del wb_destino["Brake System"]
hoja_brakesystem_destino = wb_destino.create_sheet("Brake System")

# Copiar contenido celda por celda (incluyendo estilo)
for fila in hoja_brakesystem_base.iter_rows():
    for celda in fila:
        nueva_celda = hoja_brakesystem_destino.cell(row=celda.row, column=celda.column, value=celda.value)
        if celda.has_style:
            nueva_celda.font = copy(celda.font)
            nueva_celda.border = copy(celda.border)
            nueva_celda.fill = copy(celda.fill)
            ...
            nueva_celda.protection = copy(celda.protection)
            nueva_celda.alignment = copy(celda.alignment)
print("✓ Hoja Brake System creada")

# Crear o reemplazar hoja destino Brake Type
if "Brake Type" in wb_destino.sheetnames:
    del wb_destino["Brake Type"]
hoja_braketype_destino = wb_destino.create_sheet("Brake Type")

# Copiar contenido celda por celda (incluyendo estilo)
for fila in hoja_braketype_base.iter_rows():
    for celda in fila:
        nueva_celda = hoja_braketype_destino.cell(row=celda.row, column=celda.column, value=celda.value)
        if celda.has_style:
            nueva_celda.font = copy(celda.font)
            nueva_celda.border = copy(celda.border)
            nueva_celda.fill = copy(celda.fill)
            ...
            nueva_celda.protection = copy(celda.protection)
            nueva_celda.alignment = copy(celda.alignment)
print("✓ Hoja Brake Type creada")

# Crear o reemplazar hoja destino Brake ABS
if "Brake ABS" in wb_destino.sheetnames:
    del wb_destino["Brake ABS"]
hoja_brakeabs_destino = wb_destino.create_sheet("Brake ABS")

# Copiar contenido celda por celda (incluyendo estilo)
for fila in hoja_brakeabs_base.iter_rows():
    for celda in fila:
        nueva_celda = hoja_brakeabs_destino.cell(row=celda.row, column=celda.column, value=celda.value)
        if celda.has_style:
            nueva_celda.font = copy(celda.font)
            nueva_celda.border = copy(celda.border)
            nueva_celda.fill = copy(celda.fill)
            ...
            nueva_celda.protection = copy(celda.protection)
            nueva_celda.alignment = copy(celda.alignment)
print("✓ Hoja Brake ABS creada")

# Crear o reemplazar hoja destino Spring Type
if "Spring Type" in wb_destino.sheetnames:
    del wb_destino["Spring Type"]
hoja_springtype_destino = wb_destino.create_sheet("Spring Type")

# Copiar contenido celda por celda (incluyendo estilo)
for fila in hoja_springtype_base.iter_rows():
    for celda in fila:
        nueva_celda = hoja_springtype_destino.cell(row=celda.row, column=celda.column, value=celda.value)
        if celda.has_style:
            nueva_celda.font = copy(celda.font)
            nueva_celda.border = copy(celda.border)
            nueva_celda.fill = copy(celda.fill)
            ...
            nueva_celda.protection = copy(celda.protection)
            nueva_celda.alignment = copy(celda.alignment)
print("✓ Hoja Spring Type creada")

# Crear o reemplazar hoja destino Steering System
if "Steering System" in wb_destino.sheetnames:
    del wb_destino["Steering System"]
hoja_steeringsystem_destino = wb_destino.create_sheet("Steering System")

# Copiar contenido celda por celda (incluyendo estilo)
for fila in hoja_steeringsystem_base.iter_rows():
    for celda in fila:
        nueva_celda = hoja_steeringsystem_destino.cell(row=celda.row, column=celda.column, value=celda.value)
        if celda.has_style:
            nueva_celda.font = copy(celda.font)
            nueva_celda.border = copy(celda.border)
            nueva_celda.fill = copy(celda.fill)
            ...
            nueva_celda.protection = copy(celda.protection)
            nueva_celda.alignment = copy(celda.alignment)
print("✓ Hoja Steering System creada")
...
# V To Engine
if "V To Engine" in wb_destino.sheetnames:
    del wb_destino["V To Engine"]
hoja_vtoengine_destino = wb_destino.create_sheet("V To Engine")

# Copiar encabezado
for col in range(1, hoja_vtoengine_base.max_column + 1):
    valor = hoja_vtoengine_base.cell(row=2, column=col).value
    hoja_vtoengine_destino.cell(row=1, column=col, value=valor)

print(f"Filtrando 'V to Engine' con {filtro_marca} y {filtro_vtn}")
# Filtrar y copiar filas
fila_destino = 2
for fila in range(2, hoja_vtoengine_base.max_row + 1):
    marca = hoja_vtoengine_base[f"E{fila}"].value
    tipo = hoja_vtoengine_base[f"AG{fila}"].value
    ...
    if (
        ...
        for col in range(1, hoja_vtoengine_base.max_column + 1):
            val = hoja_vtoengine_base.cell(row=fila, column=col).value
            hoja_vtoengine_destino.cell(row=fila_destino, column=col, value=val)
        fila_destino += 1
print("✓ Hoja V to Engine creada")

# V To Transmission
if "V To Transmission" in wb_destino.sheetnames:
    del wb_destino["V To Transmission"]
hoja_vtotransmission_destino = wb_destino.create_sheet("V To Transmission")

# Copiar encabezado
for col in range(1, hoja_vtotransmission_base.max_column + 1):
    valor = hoja_vtotransmission_base.cell(row=1, column=col).value
    hoja_vtotransmission_destino.cell(row=1, column=col, value=valor)

print(f"Filtrando 'V To Transmission' con {filtro_marca} y {filtro_vtn}")
# Filtrar y copiar filas
fila_destino = 2
for fila in range(2, hoja_vtotransmission_base.max_row + 1):
    marca = hoja_vtotransmission_base[f"H{fila}"].value
    tipo = hoja_vtotransmission_base[f"K{fila}"].value
    ...
    if (
        ...
        for col in range(1, hoja_vtotransmission_base.max_column + 1):
            val = hoja_vtotransmission_base.cell(row=fila, column=col).value
            hoja_vtotransmission_destino.cell(row=fila_destino, column=col, value=val)
        fila_destino += 1
print("✓ Hoja V To Transmission creada")
